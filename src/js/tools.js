...
export function convertRadarData(datas, option, seriesIndex) {
  ...
  series =
    seriesIndex !== undefined ? option.series[seriesIndex] : option.series[0];
  ...
  tags = indicator.map(function(t, index) {
    //默认标签为indicator的text属性，这里需要生成特殊的组合，即显示标签也显示每个维度的得分，
    return t.text + "：" + formatterFun(value[index], t.max);
  });
  radar.name.formatter = function(val) {
    //根据默认标签值生成特殊定制的标签值
...
export const AutoShowTip = function(chart, option, interval, params, callback) {
  ...
  if (option.radar && option.series.length === 1) {
    //单系列雷达，采用单轴轮播的方式
    autoShowInstance = new RadarAutoTip(chart, option, interval, params);
  } else {
    autoShowInstance = new CommonAutoTip(
      chart,
      option,
      interval,
      params,
...
CommonAutoTip.prototype = {
  _init: function() {
    if (this.params) {
      ...
      this.seriesIndex = this.loopSeries
        ? 0
        : this.params.seriesIndex >= 0
        ? this.params.seriesIndex
        : 0;
    }
    this._autoShowTip();
    // 鼠标在echarts图上时停止轮播
  ...
  _autoShowFn: function() {
    ...
    if (this.loopSeries && this.dataIndex === 0) {
      // 数据索引归0表示当前系列数据已经循环完
  ...
  _stopAutoShow: function() {
    ...
    if (this.chartType === "pie" || this.chartType === "radar") {
      ...
      this.chart.dispatchAction({
        type: "downplay",
        seriesIndex:
          this.seriesIndex === 0 ? this.seriesLength - 1 : this.seriesIndex - 1,
        dataIndex:
          this.dataIndex === 0 ? this.dataLength - 1 : this.dataIndex - 1
      });
    }
...
RadarAutoTip.prototype = {
  _init: function(opt) {
    ...
    opt.center = {
      pointx:
        (opt.option.radar.center &&
          Number(
            opt.option.radar.center[0].substr(
              0,
              opt.option.radar.center[0].length - 1
            )
          ) / 100) ||
        0.5,
      pointy:
        (opt.option.radar.center &&
          Number(
            opt.option.radar.center[1].substr(
              0,
              opt.option.radar.center[1].length - 1
            )
          ) / 100) ||
        0.5
    };
    ...
    for (let i = 0; i < length; i++) {
      ...
      pointData.push([
        opt.radius * m.sin(i * single) * ratio,
        opt.radius * m.cos(i * single) * ratio
      ]);
    }
    this._createLabel();
  },
  ...
  _hoverLabel: function(label, point, text, style) {
    let cssText =
      "position: absolute; display: inline-block; top: " +
      (point.y - 37) +
      "px; left:" +
      point.x +
      "px; border:" +
    ...
    cssText =
      "width: 0px; height: 0px; position: absolute; left: 50%; top: 99%; margin-left: -5px;" +
  ...
  _autoStart: function() {
    ...
    this.autoOption.intervalId = setInterval(function() {
      ...
      that.config.autoTipState &&
        !that.config.hovering &&
        that._hoverLabel(that.config.hoverLabel, showPoint, text, style);
    }, this.interval || 2000);
  },
  ...
  clearAutoShow: function() {
    ...
    if (this.config.target.getElementsByClassName("hoverLabel").length) {
      //图表刷新重置时
      this.config.target.removeChild(
        this.config.target.getElementsByClassName("hoverLabel")[0]
      );
    }
