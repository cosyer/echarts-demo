...
<template>
  ...
  <div
    ref="mountPoint"
    class="simple-list-module"
    :class="{ 'small-height':config.appendClass }"
  >
    <div
      class="simple-list-unit"
      v-if="bindData.unit"
    >
      单位：{{ bindData.unit }}
    </div>
    <div
      v-if="isReady"
      class="simple-list-shadow"
      :style="{ height:config.moduleHeight+'px' }"
    >
      <ul class="simple-list-body">
        ...
        >
          <div
            class="simple-list-info"
            :style="{ margin:config.marginStyle }"
            v-if="item.isEnable"
          >
          ...
          <div
            class="simple-list-rank"
            v-if="item.isEnable"
          >
            <i
              class="rank-num"
              :style="{ color: ++index > 3 ? config.rankColor:config.highRankColor }"
            >No.{{ index>9? index : '0'+index }}</i>
            <div class="progress-bar">
              <transition name="slide-fade">
                <span
                  class="progress-percent"
                  v-if="startAnimate"
                  v-bind:style="{width:item.formatPer}"
                ></span>
          ...
          <div
            v-if="!item.isEnable"
            v-bind:style="{height:config.lineHeight+'px'}"
            class="list-empty"
          ></div>
        </li>
      </ul>
...
export default {
  ...
  created() {
    ...
    this.configParams &&
      Object.keys(this.configParams).map((t) => {
  ...
  methods: {
    init: function () {
      ...
      config.marginStyle = this._calMargin(
        config.lineHeight - config.contentHeight
      );
      this.updateDom();
    },
    _calMargin: function (height) {
      ...
      str =
        str +
        Math.round(config.marginTop) +
        "px 0 " +
        Math.round(height - config.marginTop) +
        "px";
      return str;
    },
    /**
    ...
    _getLineHeight: function (lineHeight) {
      const config = this.config;
      let length,
        moduleHeight = config.height - config.headHeight;
      length = moduleHeight / lineHeight;
      if (moduleHeight - Math.floor(length) * lineHeight > 5) {
        //递归寻找最好的高度和长度
    ...
    _autoScroll: function () {
      var _this = this.config,
        num = _this.scrollLength,
        scrollHeight = Math.round(_this.lineHeight * _this.scrollLength);
      var $self = _this.$target;
      if (_this.data.length <= _this.scrollLength) {
        //列表过短，就不需要滚动了
        return;
      }
    ...
    myTween: function (start, end, length, tweenFun, callback) {
      var arr = [],
        count = 0,
        arrLength = (50 * length) / 1000,
        step = (end - start) / arrLength;
