...
export default {
  ...
  mounted() {
    const datas = [
      { name: "大学生", value: [30260, 35578, 41424, 44256, 45826] },
  ...
  methods: {
    initChart: function (data) {
      const target = document
          .getElementById(this.singleId)
          .querySelector("#trendAnalyseChart"),
        option = {
          tooltip: {
            trigger: "axis",
            ...
            formatter: function (param) {
              var res =
                '<div><h3 style="color:#d3edff;text-align: center;">' +
                param[0].name +
                "</h3>";
              if (param) {
                for (var i = 0; i < param.length; i++) {
                  res +=
                    '<div style="padding: 3px 0;color:#8bb8e8;text-align: left;">' +
                    param[i].seriesName +
                    ':<span style="color:#ffbf00;font-weight: bold;font-size:14px;padding: 0 5px;" >' +
                    addSeparator(param[i].value) +
                    "</span></div>";
                }
                res += "</div>";
                return res;
